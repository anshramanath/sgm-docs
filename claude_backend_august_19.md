...
- `Unpaid` — row created before payment, filtered from all user/admin views
- `Processing` — set by webhook on successful payment
- `Emailed` — set manually by admin
- `Shipped` — set manually by admin; required before fulfillment can be saved
- `Refunded` — set by webhook on full refund; also manually settable by admin

`Refunded` is a real DB status (not derived). `Partially Refunded` is derived: `refundedCents !== null && status !== "Refunded"`.
...
3. Validates `submission.depositCents === available && submission.depositCents !== 0` — errors if not equal to available OR if 0. This catches both a stale deposit value and a case where `available` is 0 (deposit fully used/refunded), preventing a silent no-op.
...
|---|---|
| `getRxOrders(brandSlug)` | Fetches all non-Unpaid orders, newest first, maps to `RxOrder` type |
...
- `STATUSES = ["Processing", "Emailed", "Shipped"]` — admin dropdown options (Refunded is webhook-set but also manually settable)
- `filterDefs` uses `[...STATUSES, "Refunded"]` + a derived "Partially Refunded" filter
- `statusLocked = order.status === "Refunded"` — Unpaid never appears (filtered by `visible`)
...
- `undoFulfillmentDisabled` — only blocked while a save is in flight; undo is available from any status (e.g. admin changed status back from Shipped but wants to clear carrier/tracking)
- After `handleStatusSave`, draft carrier/trackingNumber resets to actual saved order values — prevents stale draft appearing as if fulfillment is entered when it isn't
...
## User API (`POST /api/user/rx-orders`)

Returns all rx orders for the authenticated user scoped to `brandSlug`. Returns full form data so the user can review everything they submitted.

Fields returned: `id`, `status`, `frameName`, `frameImageSrc`, `frameColor`, `framePriceCents`, `totalPriceCents`, `depositUsedCents`, `stripeChargeCents`, `refundedCents`, `carrier`, `trackingNumber`, all prescription fields (vision type, OD/OS, PD, lens options, coatings), `comments`, `prescriptionUrl`, `headshotUrl`, `contactName/Email/Phone`, `shippingAddress`, `createdAt`.

- Uses RLS — automatically scoped to the authenticated user, no user ID filter needed
- `Unpaid` orders filtered client-side before display (`status !== "Unpaid"`)
- `data.map(...)` not `(data ?? []).map(...)` — if error is null, data is always an array
