...
2. **Six Did/Learned Threads** is the recommended organizing structure — six self-contained "what I did → what I learned" pairs spanning the whole project, each with suggested screenshots. Build the page around these rather than a chronological case-study narrative. Don't drop the frontend/state-management thread (#3) for something flashier — it's the most broadly legible "this person can build a real system" evidence in the whole project, more so than the backend-specific stories, and it got missed once already in an earlier draft.
...
*The recommended backbone for the page. Each thread is a self-contained "what I did → what I learned" pair, pulled from the strongest material in the full timeline below. These aren't meant to be used verbatim — write them in your own voice — but the substance (the specific decision, the specific insight) is what makes each one land, so keep that intact even while rewriting the wording.*

### 1. Data migration → the SKU forensics problem

**Did:** Built a pipeline that pulled ~1,650 products off three legacy WooCommerce stores and, instead of trusting any one system, audited every field against multiple sources to figure out which one was actually telling the truth — catching things like a single-letter SKU prefix (`C` for combo products) that was worth 12 products and 437 variations once fixed.

**Learned:** Data doesn't have an owner by default — you have to assign one, per field, and defend that decision. The migration got tractable the moment the question changed from "what does the data say" to "which system is allowed to be wrong here."

**Screenshots:** `database-001-004-overview.png`, `database-cart-bookmarks-tables.png`, `prod-admin-products-page.png`

### 2. Payments — designed so the same event twice is still one order

**Did:** Built order creation entirely off Stripe webhooks (never at checkout-session creation) with two independent layers of idempotency — a Stripe-side key for double-clicks, a DB-side unique constraint for duplicate webhook deliveries — plus atomic Postgres RPCs for sales counters so concurrent writes can't lose an update.

**Learned:** "It usually works" isn't a real state for anything touching money. I learned to design every write path assuming it would definitely be called twice, rather than patching it after a duplicate-order bug showed up.

**Screenshots:** `prod-checkout-page.png`, `prod-stripe-checkout.png`, `stripe-payment-setup-call.png`

### 3. The frontend nobody sees — state, streaming, and one race condition

**Did:** Rebuilt cart/bookmark/auth state as a nested provider stack (`AuthProvider` → `CartProvider` → `BookmarkProvider`) with `Map`-based state, 800ms-debounced DB writes, and Suspense-streamed category pages — then tracked down a bug where a background 401 could leave the header showing stale auth state after the session had actually expired.

**Learned:** Client state and server state aren't the same fact rendered twice — they can genuinely disagree for a moment. The fix wasn't forcing them to agree, it was modeling both truths explicitly (`isActuallySignedIn = isSignedIn && (loggedIn ?? true)`) and being deliberate about which one wins when. This is the thread most likely to get skipped in favor of flashier stories — don't skip it; it's the clearest evidence of real frontend engineering in the whole project.

**Screenshots:** `frontend-bookmarks-local-storage.png`, `backend-nextjs-debugger.png`, `claude-design-frontend-products.png`

### 4. Try Before You Buy — the feature that refused to use the cart

**Did:** Designed and shipped a full feature — deposit packages, prescription intake, Stripe checkout, admin fulfillment — deliberately kept outside the main cart/checkout code so it wouldn't force nullable branches into paths the rest of the store depended on.

**Learned:** Not every feature belongs in the shared abstraction. Sometimes the more maintainable choice is to duplicate a little rather than bend a general-purpose system around one brand's one-off flow.

**Screenshots:** `prod-tbyb-page.png`, `prod-tbyb-form.png`, `claude-design-admin-tbyb.png`

### 5. Product copy, written by a model that could actually see the product

**Did:** Built a FastMCP server that Claude Desktop drives to generate descriptions for 882 products from their actual photos, with a validator that rejects anything over 80 words, containing raw measurements, or using banned marketing language.

**Learned:** A prompt is a request; a tool is a contract. Getting real consistency across hundreds of AI-generated outputs meant the rules couldn't just live in the instructions — they had to live in code that checks the output and forces a retry.

**Screenshots:** `migration-mcp-prompt.png`, `migration-mcp-server.png`, `prod-product-page.png`

### 6. Launch — and the day the source data stopped answering

**Did:** Cut three live domains over from WordPress/SiteGround to Vercel, flipped Stripe to live mode, and ran a full clean reimport of the production database and Storage — during which all three original legacy sites became unreachable, with zero fallback available.

**Learned:** A decision made in week one (deriving image filenames deterministically from the source URL, rather than by convenience) is what saved launch week — every image got re-derived and pulled from the new storage instead of the dead source. The value of a boring, disciplined early decision often doesn't show up until months later, under pressure, when you didn't know you'd need it.
